...
public func allPass<T,U>
    (_ passFunc: (T?) -> Bool) -> NonNilMatcherFunc<U> where U: Sequence, U.Iterator.Element == T {
        return allPass("pass a condition", passFunc)
}

public func allPass<T,U>
    (_ passName: String, _ passFunc: (T?) -> Bool) -> NonNilMatcherFunc<U> where U: Sequence, U.Iterator.Element == T {
    ...
    public class func allPassMatcher(_ matcher: NMBObjCMatcher) -> NMBObjCMatcher {
        return NMBObjCMatcher(canMatchNil: false) { actualExpression, failureMessage in
            ...
            if let value = actualValue as? NSFastEnumeration {
                ...
                while let obj:AnyObject = generator.next() as AnyObject? {
